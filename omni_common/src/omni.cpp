...
public:
  ...
  void init(OmniState *s) {
    ros::param::param(std::string("~omni_name"), omni_name,
        std::string("phantom"));
std::cout<<"ros::param::param(std::string(~omni_name)omni_namestd::string(phantom))"<<std::endl;
  ...
  void publish_omni_state() {
    ...
    // std::cout<<"joint_state.name[0]"<<joint_state.name[0]<<std::endl;
    // std::cout<<"joint_state.position[0]"<<joint_state.position[0]<<std::endl;
    joint_state.name[1] = "shoulder";
    joint_state.position[1] = state->thetas[2];
    // std::cout<<"joint_state.name[1]"<<joint_state.name[1]<<std::endl;
    // std::cout<<"joint_state.position[1]"<<joint_state.position[1]<<std::endl;
    joint_state.name[2] = "elbow";
    joint_state.position[2] = state->thetas[3];
    // std::cout<<"joint_state.name[2]"<<joint_state.name[2]<<std::endl;
    // std::cout<<"joint_state.position[2]"<<joint_state.position[2]<<std::endl;
    joint_state.name[3] = "yaw";
    joint_state.position[3] = -state->thetas[4] + M_PI;
    // std::cout<<"joint_state.name[3]"<<joint_state.name[3]<<std::endl;
    // std::cout<<"joint_state.position[3]"<<joint_state.position[3]<<std::endl; 
    joint_state.name[4] = "pitch";
    joint_state.position[4] = -state->thetas[5] - 3*M_PI/4;
    // std::cout<<"joint_state.name[4]"<<joint_state.name[4]<<std::endl;
    // std::cout<<"joint_state.position[4]"<<joint_state.position[4]<<std::endl; 
    joint_state.name[5] = "roll";
    joint_state.position[5] = -state->thetas[6] - M_PI;
    // std::cout<<"joint_state.name[5]"<<joint_state.name[5]<<std::endl;
    // std::cout<<"joint_state.position[5]"<<joint_state.position[5]<<std::endl;
    ...
    pose_stamped.pose.position.x = state->position[0];   //was 0.03 to end of phantom
...
// std::cout<<"state->buttons[0]"<<state->buttons[0]<<std::endl;
      if ((state->buttons[0] == state->buttons[1])
          and (state->buttons[0] == 1)) {
        state->lock = !(state->lock);
        // std::cout<<"state->lock"<<state->lock<<std::endl;
      }
...
HDCallbackCode HDCALLBACK omni_state_callback(void *pUserData) {
  ...
  // std::cout<<"omni_state->pos_hist2"<<"  "<<omni_state->pos_hist2<<std::endl;
  omni_state->pos_hist1 = omni_state->position;
  // std::cout<<"omni_state->position"<<"  "<<omni_state->position<<std::endl;
  ...
  // std::cout<<"omni_state->out_vel1"<<omni_state->out_vel1<<std::endl;
  // std::cout<<"omni_state->out_vel2"<<omni_state->out_vel2<<std::endl;
  // std::cout<<"omni_state->out_vel3"<<omni_state->out_vel3<<std::endl;
  if (omni_state->lock == true) {
    // omni_state->force = 0.04 * (omni_state->lock_pos - omni_state->position)
    //     - 0.001 * omni_state->velocity;
          //  std::cout<<"omni_state->force"<<omni_state->force<<std::endl;
  }
  ...
  float t[7] = { 0., omni_state->joints[0], omni_state->joints[1],
      ...
      omni_state->rot[1], omni_state->rot[2] };
        std::cout<<"omni_state->rot[0]"<<"  "<<omni_state->rot[0]<<std::endl;
        std::cout<<"omni_state->rot[1]"<<"  "<<omni_state->rot[1]<<std::endl;
        std::cout<<"omni_state->rot[2]"<<"  "<<omni_state->rot[2]<<std::endl;
        std::cout<<"omni_state->rot[3]"<<"  "<<omni_state->rot[3]<<std::endl;
        std::cout<<"omni_state->rot[4]"<<"  "<<omni_state->rot[4]<<std::endl;
        std::cout<<"omni_state->rot[5]"<<"  "<<omni_state->rot[5]<<std::endl;
  for (int i = 0; i < 7; i++)
